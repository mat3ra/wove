...
`github:mat3ra/wove#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/wove/releases/download/wip-<short-sha>/wove.tgz
```
...
```bash
npm run mat3ra:install -- wove wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/wove@<release-url>` with
